...
import { CalendarCheck, Users, Search, Rocket, Badge } from "lucide-react";
...
const Home = () => {
  useEffect(() => {
    ...
    if (!hasShownToast) {
      toast.success("Welcome to Event Tracker!", {
        position: "top-right",
        autoClose: 5000,
...
return (
  <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#121212] to-[#1a1a1a] text-white">
    ...
    <section className="flex flex-col items-center justify-center text-center py-24 px-6">
      <motion.dev
      >
        <Badge className="absolute bottom-10 right-10 w-24 h-24 text-blue-400" />
        <Badge className="absolute bottom-10 left-10 w-24 h-24 text-yellow-400" />
    ...
    <section className="py-16 px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
        {[
          ...
          <motion.div
            key={index}
            className="p-6 bg-[#1e1e1e] bg-opacity-90 backdrop-blur-lg border-yellow-200 border-4 rounded-xl shadow-lg  hover:shadow-blue-500/50"
